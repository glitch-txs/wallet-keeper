...
const NetworkSelector: React.FC<NetworkSelectorProps> = ({ options }) => {
	...
	const filteredOptions = options.filter((chain) => chain.name.toLowerCase().includes(searchTerm.toLowerCase()))
				...
				<ul className={styles.dropdown}>
					{filteredOptions.length > 0 ? (
						filteredOptions.map((chain, index) => (
							<li
								key={index}
								className={`${styles.dropdownItem} ${chain.name === activeChain.name ? styles.active : ''}`}
